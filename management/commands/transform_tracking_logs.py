...
def transform_tracking_logs(
    ...
    # 添加这一行
    #global total_logs_processed, total_logs_transformed
            ...
            for chunk in chunks:
                ...
                for char in chunk:
                    if char == "\n" and line:
                        ...
                        result = sender.transform_and_queue(line)
                        #if result:
                        #    total_logs_transformed += 1
                        line = ""
                    else:
                        line += char
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def transform_tracking_logs_for_test(
            source,
            source_container,
            source_prefix,
            sender
        ):
#            nonlocal total_logs_processed, total_logs_transformed
            container = source.get_container(container_name=source_container)
            ...
            total_logs_sent = sender.finalize()
            return total_logs_sent

    #    total_logs_sent = transform_tracking_logs(
    #        source_driver,
    #        source_container,
    #        source_prefix,
    #        sender
    ...
    #    print(f"Total logs transformed: {total_logs_transformed}")
    #    print(f"Total logs sent to LRS: {total_logs_sent}")
