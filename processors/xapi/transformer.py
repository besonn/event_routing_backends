...
class XApiTransformer(BaseTransformerMixin):
    ...
    def transform(self):
        ...
        #print("********************")
        statement = Statement(**transformed_props)
        #statement.timestamp = statement.timestamp.isoformat()
        #format_str = statement.timestamp.strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
        #print(format_str)
        #statement.timestamp = datetime.strptime(format_str, '%Y-%m-%dT%H:%M:%SZ')
        #print("********************")
        return statement
    ...
    def get_timestamp(self):
        ...
        Returns:
            str
        """
        # return_time = self.get_data('timestamp') or self.get_data('time')
        # return return_time[0:19] + 'Z'
